...
- Read `AGENTS.local.md` after this file when it exists.
- Read every unscoped rule in `.agents/rules/` before repository work.
- Before reading or changing a path, apply any rule whose `paths:` frontmatter
  matches that path.
- Load a skill from `.agents/skills/` when its description matches the task.
...
- **UPPERCASE**: Only for standard files (`README.md`, `CHANGELOG.md`, `AGENTS.md`, `CLAUDE.md`, `LICENSE`)
...
This repository uses `bd` for issue tracking. Read `AGENTS.beads.md` before
issue-tracking work.
